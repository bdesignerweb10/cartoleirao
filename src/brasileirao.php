<?php
	require_once('header.php');
?>
<main>
	<div class="container table-br">
		<div class="row">
			<div class="col-sm-8">
				<h5 class="headline">Tabela</h5>
				<table class="table table-hover">
				  <thead>
				    <tr>
				      <th>#</th>	
				      <th>Clube</th>
				      <th>P</th>
				      <th>J</th>
				      <th>V</th>
				      <th>E</th>
				      <th>D</th>
				      <th>GP</th>
				      <th>GC</th>
					  <th>SG</th>
					  <th>%</th>
				    </tr>
				  </thead>
				  <tbody>
				    <tr>
				      <th scope="row" class="text-primary">1</th>
				      <td>Corinthians</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-primary">2</th>
				      <td>Palmeiras</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-primary">3</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-primary">4</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-info">5</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-info">6</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">7</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">8</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">9</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">10</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">11</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-success">12</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row">13</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row">14</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row">15</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row">16</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-danger">17</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-danger">18</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-danger">19</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				    <tr>
				      <th scope="row" class="text-danger">20</th>
				      <td>São Paulo</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				      <td>3</td>
				    </tr>
				  </tbody>
				</table>
				<ul class="nav legend">
			  		<li class="nav-item">
			  			<span class="badge badge-primary"><i class="fas fa-square text-primary"></i></span> Libertadores
			  		</li>
			  		<li class="nav-item">
			  			<span class="badge badge-info"><i class="fas fa-square text-info"></i></span> Pré-Libertadores
			  		</li>
			  		<li class="nav-item">
			  			<span class="badge badge-success"><i class="fas fa-square text-success"></i></span> Sulamericana
			  		</li>
			  		<li class="nav-item">
			  			<span class="badge badge-danger"><i class="fas fa-square text-danger"></i></span> Rebaixados
			  		</li>
			  	</ul>
			</div>
			<div class="col-sm-4">
				<h5 class="headline">Jogos</h5>
				<table class="table table-hover table-jogos">
				  <thead>
				    <tr>
				      <th><a href="#"><i class="fas fa-chevron-left"></i></a></th>
				      <th>1º Rodada</th>
				      <th><a href="#"><i class="fas fa-chevron-right"></i></a></th>
				    </tr>
				  </thead>
				</table>
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card --> 
				<div class="card">
				  <div class="card-body">				    
				    <p class="game-info"><strong>SÁB 27/04/2019</strong> MORUMBI <strong>16:00</strong></p>
				    <div class="game">
				    	<span>SPO</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/sao-paulo.svg" width="30" height="30">
				    	<span class="vs">x</span>
				    	<img src="https://s.glbimg.com/es/sde/f/equipes/2018/03/11/botafogo.svg" width="30" height="30">
				    	<span>BOT</span>
				    </div>
				  </div>
				</div><!-- card -->     
			</div>
		</div><!-- row -->
	</div><!-- container -->
</main>
<?php
	require_once('footer.php');
?>